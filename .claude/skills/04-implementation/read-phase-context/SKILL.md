---
name: read-phase-context
description: "Resolve and read the accepted Dalgo consulting context for Phase 4 skills, including me_goals.md, KPI Framework, dbt_plan.md, er_diagram.md, source YAMLs, dbt repo path, and metadata."
---

# Read Phase Context

Resolve and read the engagement context needed by Phase 4 skills.

Use this skill at the start of the `modify-requirements` skill, the `dbt-edit-plan` skill, the `investigate-issue` skill, the `write-dbt-staging` skill, the `write-dbt-intermediate` skill, the `write-dbt-marts` skill, the `generate-data-dictionary` skill, and the `finalize-dbt-project` skill.
...
5. Otherwise ask:
   > "Which engagement should this skill use?"
...
If the KPI Framework Sheet ID is available in metadata, invoke the `read-kpi-framework` skill first to refresh the local Markdown and JSON copies.
...
Make these values available to the calling skill:

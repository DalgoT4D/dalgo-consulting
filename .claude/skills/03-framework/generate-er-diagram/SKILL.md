...
description: "Generate er_diagram.md from the KPI Framework, dbt_plan.md, me_goals.md, and enriched source YAMLs, documenting entities, grains, relationships, join paths, and cardinality risks."
---

# Generate Er Diagram

Creates `er_diagram.md` from `me_goals.md`, the KPI Framework, `dbt_plan.md`, and enriched source YAMLs.
Use this after the `plan-dbt-architecture` skill.
...
Resolve `{engagement}` using the same rules as the `plan-dbt-architecture` skill.
...
Invoke the `read-kpi-framework` skill to refresh:

---
name: plan-dbt-architecture
description: "Create dbt_plan.md from me_goals.md, the KPI Framework, and enriched source YAMLs, designing staging, intermediate, and chart-ready marts_ models for Dalgo dashboards, metrics, filters, drilldowns, and alerts."
---

# Plan Dbt Architecture

Creates `dbt_plan.md` from `me_goals.md`, the KPI Framework Sheet, and enriched source YAMLs.
Use this after the `build-kpi-framework` skill has produced the normalized KPI Framework artifacts.
...
Treat the skill input as optional source YAML paths.
...
1. If the skill is run from inside `workdocs/consulting/{engagement}/...`, use that engagement.
...
4. Otherwise ask:
   > "Which engagement should the `plan-dbt-architecture` skill use?"
...
- `{me_goals_path}`
- all `{sources_paths}`
- `workdocs/consulting/{engagement}/framework/kpi_framework.md` and `.json`, after refreshing them via the `read-kpi-framework` skill

---

## Step 1 — Refresh The KPI Framework Local Copy

Invoke the `read-kpi-framework` skill.
...
Then print:
> "Review `dbt_plan.md`. Once accepted, run the `generate-er-diagram` skill."

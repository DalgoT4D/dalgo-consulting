...
Use this skill directly from dbt-wizard. It should suggest a fix when evidence supports one, or state that the root cause could not be found while preserving useful investigative results.
...
- Resolve the dbt profile from `{dbt_repo_path}/profiles.yml`, `{dbt_repo_path}/profiles.yaml`, or `~/.dbt/profiles.yml`.
- Resolve environment variables.
- Run `dbt debug`.
- Confirm the active adapter is Postgres-compatible for v1.
- Build the warehouse connection for investigative SQL.
...
If the cause is found, include evidence and recommend whether to invoke the `dbt-edit-plan` skill.
...
```json
{
  ...
  "queries": ".../queries.sql",
  "recommended_next_step": "dbt-edit-plan|user_input|no_action",
  "affected_models": [],
  "affected_kpis": []
}
```

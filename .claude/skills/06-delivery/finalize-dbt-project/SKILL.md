---
name: finalize-dbt-project
description: "Run final dbt validation, SQLFluff linting, dbt YAML documentation review, dbt docs generation, metadata updates, and explicit GitHub delivery gates for a Dalgo dbt project."
---

# Finalize Dbt Project

Runs the final dbt verification, linting, documentation, and PR-readiness checks.

Use this after the `generate-data-dictionary` skill.

Model and column documentation/tests should already be written directly in dbt YAML files by the Phase 4 skills.
...
Use the repo's existing SQLFluff invocation if documented. Otherwise try:
...
For each repo with a GitHub remote and relevant changes, invoke the `github-delivery` skill with the repo path, intended file list, proposed branch, commit message, and PR base.

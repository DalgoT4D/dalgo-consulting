# /write_staging — Write And Verify Staging Models
...
Treat text after `/write_staging` as optional source YAML paths or a dbt selector.
...
- the working tree has uncommitted changes in files this command would edit and the user has not confirmed they are expected.
...
Write the YAML files directly as part of this command; do not leave documentation/tests as prose-only recommendations.
...
```json
{
  "staging": {
    "status": "complete",
    "models": ["stg_..."],
    "last_run_command": "dbt run --select ...",
    "notes": []
  }
}
```
...
Then print:
> "Next → run `/write_intermediate` after reviewing staging outputs."

# /build_kpi_sheet — Build The KPI Framework Sheet

Creates or updates the multi-tab KPI Framework Sheet for Phase 3.
Use this after `/explore_data`, once `me_goals.md` exists and the relevant `source.yml` / `sources.yml` files have been enriched.
...
Treat the text after `/build_kpi_sheet` as optional source YAML paths. Run through each step in order.

---

## Step 0 — Resolve Inputs

If the user passed one or more paths after `/build_kpi_sheet`, treat them as `{sources_paths}`.
...
If the files do not appear enriched by `/explore_data`, warn the user and ask whether to continue. The KPI Framework should usually be built only after exploration.
...
Invoke the `read_kpi_framework` skill with:
...
Then print:
> "Review the KPI Framework Sheet with the consultant/client. Once accepted, run `/dbt_plan`."

#!/usr/bin/env python3
"""
data_dictionary_sheet.py — Create, write, and read Dalgo Data Dictionary Sheets.

Usage:
    python3 scripts/data_dictionary_sheet.py create --title "Data Dictionary - GiveDo"
    python3 scripts/data_dictionary_sheet.py write --sheet-id SHEET_ID --input-json data_dictionary.json
    python3 scripts/data_dictionary_sheet.py read --sheet-id SHEET_ID --format markdown
...
def ensure_tabs(sheets, sheet_id: str) -> dict[str, int]:
    ...
    if "Sheet1" in titles and "Data Dictionary" not in titles:
        requests.append(
            {
                "updateSheetProperties": {
                    "properties": {"sheetId": titles["Sheet1"], "title": "Data Dictionary"},
                    "fields": "title",
                }
            }
        )
        titles["Data Dictionary"] = titles.pop("Sheet1")
                    ...
                    "repeatCell": {
                        ...
                        "cell": {
                            "userEnteredFormat": {
                                ...
                                "backgroundColor": {"red": 0.89, "green": 0.95, "blue": 0.89},
                            }
                        },
...
def create_spreadsheet(sheets, drive, title: str, share_with: list[str], metadata_output: Path | None) -> None:
    spreadsheet = (
        sheets.spreadsheets()
        .create(body={"properties": {"title": title}, "sheets": [{"properties": {"title": "Data Dictionary"}}]})
        .execute()
    )
    ...
    if metadata_output:
        ...
        if metadata_output.exists():
            try:
                ...
                existing = {}
        existing["data_dictionary_sheet"] = {
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create, write, and read Dalgo Data Dictionary Sheets.")
    parser.add_argument("--key-file", type=Path, default=DEFAULT_KEY_FILE)
    subparsers = parser.add_subparsers(dest="command", required=True)

    create = subparsers.add_parser("create", help="Create a new Data Dictionary Sheet.")
    create.add_argument("--title", required=True)
    create.add_argument("--share-with", action="append", default=[])
    create.add_argument("--metadata-output", type=Path)

    write = subparsers.add_parser("write", help="Write Data Dictionary JSON rows to an existing sheet.")
    write.add_argument("--sheet-id", required=True)
    write.add_argument("--input-json", type=Path, required=True)

    read = subparsers.add_parser("read", help="Read an existing Data Dictionary Sheet.")
